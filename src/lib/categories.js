...
import {
  ...
  QUERY_ALL_CATEGORIES_BY_PARENT,
...
/**
 * ticketCategoryPathBySlug
 */

export function ticketCategoryPathBySlug(slug) {
  return `/tickets/categories/${slug}`;
}
...
export async function getCategoryByParent(parentId) {
  const apolloClient = getApolloClient();
  ...
  try {
    categoryData = await apolloClient.query({
      query: QUERY_ALL_CATEGORIES_BY_PARENT,
      variables: {
        parentId,
      },
    });
  } catch (e) {
    console.log(`[categories][getCategoryByParent] Failed to query category data: ${e.message}`);
    throw e;
  }

  const categories = categoryData?.data.categories.edges.map(({ node = {} }) => node);
