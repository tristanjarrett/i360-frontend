...
export function constructPageMetadata(defaultMetadata = {}, pageMetadata = {}, options = {}) {
  ...
  const helmet = 'data-react-helmet="true';
...
export async function getRedirects() {
  ...
  if (process.env.WORDPRESS_PLUGIN_SEO === true) {
    try {
      redirectsData = await apolloClient.query({
        query: QUERY_SEO_DATA,
      });
    } catch (e) {
      console.log(`[site][getRedirects] Failed to query SEO plugin: ${e.message}`);
      console.log('Is the SEO Plugin installed? If not, disable WORDPRESS_PLUGIN_SEO in next.config.js.');
      throw e;
    }

    const redirects = redirectsData?.data?.seo.redirects;
