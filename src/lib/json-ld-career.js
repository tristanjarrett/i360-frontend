...
import { careerPathBySlug } from 'lib/careers';
import { pagePathBySlug } from 'lib/pages';

import config from '../../package.json';

export function ArticleJsonLd({ career = {}, siteTitle = '' }) {
  const { homepage = '', faviconPath = '/favicon.ico' } = config;
  const { title, slug, excerpt, date, author, categories, modified, featuredImage } = career;
  const path = careerPathBySlug(slug);
